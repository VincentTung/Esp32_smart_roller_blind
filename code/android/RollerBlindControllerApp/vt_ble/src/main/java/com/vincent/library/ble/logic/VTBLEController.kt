...
class VTBLEController(
    ...
    private var bleVTBLECallback: VTBLECallback = DefaultBLECallback()
    ...
    fun scan(callback: VTBLECallback) {
        ...
        logd("开始扫描周围的BLE设备...")
        logd("目标设备名称: $bleDeviceName")
        logd("目标服务UUID: $bleDeviceServiceID")
        ...
        try {
            ...
            logd("添加服务UUID过滤器: $bleDeviceServiceID")
        } catch (e: IllegalArgumentException) {
            logd("服务UUID格式无效，跳过UUID过滤器: $bleDeviceServiceID")
        }
    ...
    fun connect(callback: VTBLECallback) {
        ...
        if (hasDeviceAddress()) {
            logd("=== 尝试快速重连模式 ===")
            logd("使用保存的设备地址: $bleDeviceAddress")
    ...
    private fun checkAndRequestBonding(device: BluetoothDevice) {
        ...
        if (BLEDeviceManager.shouldBondDevice(device)) {
            logd("设备需要配对，开始配对流程")
    ...
    private fun handleBondStateChange(device: BluetoothDevice?, bondState: Int) {
        ...
        when (bondState) {
            ...
            BluetoothDevice.BOND_BONDED -> {
                ...
                // 保存配对成功的设备信息
                BLEDeviceManager.saveConnectedDevice(device)
