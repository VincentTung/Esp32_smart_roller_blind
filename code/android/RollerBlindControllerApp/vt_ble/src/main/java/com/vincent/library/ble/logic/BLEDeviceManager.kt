...
object BLEDeviceManager {
    ...
    fun saveConnectedDevice(device: BluetoothDevice) {
        try {
            ...
            val currentCount = MMKVUtil.getInt(MMKV_ID, KEY_DEVICE_CONNECT_COUNT, 0)
            
            // 保存设备信息
            MMKVUtil.putString(MMKV_ID, KEY_DEVICE_NAME, device.name ?: "")
            MMKVUtil.putString(MMKV_ID, KEY_DEVICE_ADDRESS, device.address)
            MMKVUtil.putInt(MMKV_ID, KEY_DEVICE_BOND_STATE, device.bondState)
            MMKVUtil.putLong(MMKV_ID, KEY_DEVICE_LAST_CONNECT_TIME, currentTime)
            MMKVUtil.putInt(MMKV_ID, KEY_DEVICE_CONNECT_COUNT, currentCount + 1)
    ...
    fun saveConnectedDevice(deviceName: String?, deviceAddress: String) {
        try {
            ...
            // 保存设备信息
            MMKVUtil.putString(MMKV_ID, KEY_DEVICE_NAME, deviceName ?: "")
            MMKVUtil.putString(MMKV_ID, KEY_DEVICE_ADDRESS, deviceAddress)
            MMKVUtil.putLong(MMKV_ID, KEY_DEVICE_LAST_CONNECT_TIME, currentTime)
            MMKVUtil.putInt(MMKV_ID, KEY_DEVICE_CONNECT_COUNT, currentCount + 1)
    ...
    fun getLastConnectedDeviceAddress(): String? {
        val address = MMKVUtil.getString(MMKV_ID, KEY_DEVICE_ADDRESS)
    ...
    fun getLastConnectedDeviceName(): String? {
        val name = MMKVUtil.getString(MMKV_ID, KEY_DEVICE_NAME)
    ...
    fun getLastConnectedDeviceBondState(): Int {
        val bondState = MMKVUtil.getInt(MMKV_ID, KEY_DEVICE_BOND_STATE, BluetoothDevice.BOND_NONE)
    ...
    fun getDeviceConnectCount(): Int {
        val count = MMKVUtil.getInt(MMKV_ID, KEY_DEVICE_CONNECT_COUNT, 0)
    ...
    fun getDeviceLastConnectTime(): Long {
        val time = MMKVUtil.getLong(MMKV_ID, KEY_DEVICE_LAST_CONNECT_TIME, 0L)
    ...
    fun clearSavedDevice() {
        try {
            MMKVUtil.removeKey(MMKV_ID, KEY_DEVICE_NAME)
            MMKVUtil.removeKey(MMKV_ID, KEY_DEVICE_ADDRESS)
            MMKVUtil.removeKey(MMKV_ID, KEY_DEVICE_BOND_STATE)
            MMKVUtil.removeKey(MMKV_ID, KEY_DEVICE_LAST_CONNECT_TIME)
            MMKVUtil.removeKey(MMKV_ID, KEY_DEVICE_CONNECT_COUNT)
    ...
    fun hasSavedDevice(): Boolean {
        val hasAddress = MMKVUtil.containsKey(MMKV_ID, KEY_DEVICE_ADDRESS)
