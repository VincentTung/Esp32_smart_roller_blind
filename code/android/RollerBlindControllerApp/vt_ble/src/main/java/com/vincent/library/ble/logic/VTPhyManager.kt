...
import com.vincent.library.ble.config.PHY_NEGOTIATION_RETRY_COUNT
import com.vincent.library.ble.config.PHY_NEGOTIATION_RETRY_DELAY
...
class VTPhyManager(private val context: Context) {
    ...
    fun startPhyNegotiation(gatt: BluetoothGatt?, callback: VTBLECallback) {
        ...
        // 设置PHY协商超时
        phyNegotiationTimeoutJob = CoroutineUtil.delayInScope(SCOPE_NAME,
            PHY_NEGOTIATION_TIMEOUT
        ) {
            logd("=== PHY协商超时 ===")
