...
// RF引脚状态
int lastRFOpenState = 0;
int lastRFCloseState = 0;
unsigned long lastRFOpenTime = 0;
unsigned long lastRFCloseTime = 0;
const unsigned long RF_DEBOUNCE_TIME = 200;  // 200ms防抖时间
...
// 处理BLE命令队列（优先级最高）
  ...
  //检查rf信息
  int open = digitalRead(RF_D2_PIN);
  int close = digitalRead(RF_DO_PIN);
  unsigned long currentTime = millis();
  
  // 处理RF打开信号
  if (open == 1) {
    // 检查防抖：状态变化或时间间隔足够
    if (lastRFOpenState != 1 || (currentTime - lastRFOpenTime) > RF_DEBOUNCE_TIME) {
      lastRFOpenState = 1;
      lastRFOpenTime = currentTime;
      
      // 检查窗帘状态，避免重复操作
      if (!isFullyRolledUp && !motorRunning) {
        DEBUG_PRINTLN("RF信号：打开窗帘");
        resetCurtainState();
        rollUpCurtain();
      } else if (isFullyRolledUp) {
        DEBUG_PRINTLN("RF信号：窗帘已经完全升起，忽略");
      } else if (motorRunning) {
        DEBUG_PRINTLN("RF信号：电机正在运行，忽略打开命令");
      }
    }
  } else {
    // 状态变为0时重置
    if (lastRFOpenState == 1) {
      lastRFOpenState = 0;
    }
  }
  ...
  if (close == 1) {
    // 检查防抖：状态变化或时间间隔足够
    if (lastRFCloseState != 1 || (currentTime - lastRFCloseTime) > RF_DEBOUNCE_TIME) {
      lastRFCloseState = 1;
      lastRFCloseTime = currentTime;
      
      // 检查窗帘状态，避免重复操作
      if (!isFullyRolledDown && !motorRunning) {
        DEBUG_PRINTLN("RF信号：关闭窗帘");
        resetCurtainState();
        layDownCurtain();
      } else if (isFullyRolledDown) {
        DEBUG_PRINTLN("RF信号：窗帘已经完全放下，忽略");
      } else if (motorRunning) {
        DEBUG_PRINTLN("RF信号：电机正在运行，忽略关闭命令");
      }
    }
  } else {
    // 状态变为0时重置
    if (lastRFCloseState == 1) {
      lastRFCloseState = 0;
    }
  }
